...
grad_img = ax_beaker.imshow(gradient, extent=[0, 2, 0, H], aspect='auto', cmap='coolwarm', alpha=0.4)
...
def update_physics():
    ...
    if y <= 0:
        ...
        if state_in_air:
            last_W = W_accum
            last_Qin = Qin_accum
            last_eff = (last_W / last_Qin) * 100 if last_Qin > 0 else 0
            W_accum = 0.0
            Qin_accum = 0.0
            state_in_air = False
